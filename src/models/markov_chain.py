...
class MarkovTrader:
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader) -> MarkovChainModel:
...
def add_markov_predictions(data: pd.DataFrame, 
                          markov_trader: MarkovTrader) -> pd.DataFrame:
    ...
    for i in range(len(data) - markov_trader.sequence_length):
        ...
        sequences.append({
            ...
            'bull_probability': probs[3] + probs[4],  # Probability of bullish states
            'bear_probability': probs[0] + probs[1]   # Probability of bearish states
        })
    ...
    data = data.copy()
    data.loc[markov_trader.sequence_length:, 'predicted_state'] = predictions_df['predicted_state'].values
    data.loc[markov_trader.sequence_length:, 'bull_probability'] = predictions_df['bull_probability'].values
    data.loc[markov_trader.sequence_length:, 'bear_probability'] = predictions_df['bear_probability'].values
