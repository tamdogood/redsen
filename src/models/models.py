...
class DeepTradingModel:
    ...
    def prepare_data(self, data: pd.DataFrame) -> Tuple[DataLoader, DataLoader, DataLoader]:
        ...
        # Split features
        sentiment_columns = [
            'comment_sentiment_avg', 'bullish_comments_ratio',
            'bearish_comments_ratio', 'sentiment_momentum',
            'sentiment_volatility', 'sentiment_strength'
        ]
        
        technical_columns = [col for col in data.columns 
...
if __name__ == "__main__":
    # Initialize model
    trading_model = DeepTradingModel(
        sequence_length=10,
        batch_size=32,
        num_layers=3
    )
