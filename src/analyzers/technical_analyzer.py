...
class TechnicalAnalyzer:
    ...
    def _calculate_volatility_metrics(self, hist: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if len(returns) > 0:
                # Daily volatility
                metrics["volatility_daily"] = round(
                    returns.std() * np.sqrt(252) * 100, 2
                )
            ...
            if len(hist) >= 14:
                try:
                    ...
                    metrics["atr"] = round(
                        (hist["High"] - hist["Low"]).rolling(window=14).mean().iloc[-1],
                        4,
                    )
            return metrics
    ...
    def _identify_price_trend(self, prices: pd.Series) -> str:
        ...
        try:
            ...
            sma50 = (
                prices.rolling(window=min(50, len(prices))).mean().iloc[-1]
                if len(prices) >= 50
                else None
            )
            sma200 = (
                prices.rolling(window=min(200, len(prices))).mean().iloc[-1]
                if len(prices) >= 200
                else None
            )
    ...
    def _get_sector_performance(self, ticker: str) -> Dict:
        ...
        try:
            ...
            if hasattr(ticker_details, "standard_industrial_classification"):
                industry = ticker_details.standard_industrial_classification.get(
                    "industry_title", ""
                )
            ...
            sector_metrics = {
                ...
                "sector_performance_1m": round(
                    ((etf_aggs["close"].iloc[-1] / etf_aggs["close"].iloc[0]) - 1)
                    * 100,
                    2,
                ),
                ...
                "sector_volatility": round(
                    etf_aggs["close"].pct_change().std() * np.sqrt(252) * 100, 2
                ),
            }
            ...
            try:
                ...
                sector_metrics["sector_momentum"] = round(
                    etf_aggs["close"].pct_change(5).mean() * 100, 2  # 5-day momentum
                )
                ...
                sector_metrics["sector_volume_trend"] = round(
                    (recent_volume / avg_volume - 1) * 100, 2
                )

                # Calculate sector volatility trend
                recent_volatility = (
                    etf_aggs["close"].pct_change().tail(5).std() * np.sqrt(252) * 100
                )
                sector_metrics["sector_volatility_trend"] = round(
                    recent_volatility - sector_metrics["sector_volatility"], 2
                )
                ...
                strength_factors = [
                    ...
                    etf_aggs["close"].iloc[-1]
                    > sector_metrics["sector_sma20"],  # Above SMA20
                    sector_metrics["sector_relative_strength"] > 1,  # Strong vs market
                ]

                sector_metrics["sector_strength_score"] = round(
                    (sum(1 for x in strength_factors if x) / len(strength_factors))
                    * 100,
                    2,
                )
