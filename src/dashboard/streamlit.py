...
def load_sentiment_data(start_date, end_date):
    ...
    try:
        ...
        if response.data:
            ...
            numeric_columns = [
                'comment_sentiment_avg', 'base_sentiment', 'submission_sentiment',
                'bullish_comments_ratio', 'bearish_comments_ratio', 'composite_score'
            ]
...
def load_post_data(tickers, start_date, end_date):
    ...
    try:
        ...
        post_ids_response = supabase.table("post_tickers") \
            .select("post_id") \
            .in_("ticker", tickers) \
        ...
        # Get post details
        posts_response = supabase.table("reddit_posts") \
            .select("*") \
        ...
        # Convert numeric columns and handle NaN values
        if not df.empty:
            numeric_cols = ['avg_sentiment', 'avg_base_sentiment', 'avg_weighted_sentiment']
...
def create_sentiment_heatmap(df, metric, title):
    ...
    try:
        ...
        pivot_data = df.pivot_table(
            ...
            columns='ticker',
            aggfunc='mean'
        ).fillna(0)  # Fill NaN values with 0 for visualization
...
def main():
    ...
    with tab2:
        ...
        active_stocks = df.groupby('ticker').agg({
            'num_comments': 'sum',
            'comment_sentiment_avg': 'mean',
            'bullish_comments_ratio': 'mean',
            'bearish_comments_ratio': 'mean',
            'composite_score': 'mean'
        }).fillna(0).sort_values('num_comments', ascending=False)
        ...
        min_comments = df['num_comments'].mean() if not df['num_comments'].isna().all() else 0
        qualified_stocks = active_stocks[active_stocks['num_comments'] > min_comments].copy()
        
        # Calculate sentiment ratio for better classification
        qualified_stocks['sentiment_ratio'] = qualified_stocks['bullish_comments_ratio'] / (
            qualified_stocks['bearish_comments_ratio'] + 0.0001  # Avoid division by zero
        )
        
        # Identify bullish stocks first
        bullish_stocks = qualified_stocks[
            qualified_stocks['sentiment_ratio'] > 1.5  # Clearly bullish
        ].sort_values('bullish_comments_ratio', ascending=False).head(10)
        ...
        bearish_stocks = remaining_stocks[
            remaining_stocks['sentiment_ratio'] < 0.67  # Clearly bearish (1/1.5)
        ].sort_values('bearish_comments_ratio', ascending=False).head(10)
        ...
        if not bullish_stocks.empty:
            ...
            fig_bullish.add_trace(
                go.Scatter(
                    x=bullish_stocks.index,
                    y=bullish_stocks['num_comments'] / bullish_stocks['num_comments'].max(),
            ...
            bullish_details = bullish_stocks[[
                'num_comments', 'comment_sentiment_avg', 
        ...
        if not bearish_stocks.empty:
            ...
            fig_bearish.add_trace(
                go.Scatter(
                    x=bearish_stocks.index,
                    y=bearish_stocks['num_comments'] / bearish_stocks['num_comments'].max(),
                    name='Relative Volume',
                    yaxis='y2',
                    line=dict(color='black', dash='dot')
                )
        ...
        if selected_tickers:
            ...
            if not posts_df.empty:
                for _, post in posts_df.iterrows():
                    with st.expander(f"{post.get('title', 'No Title')} ({post.get('subreddit', 'Unknown')})"):
                        ...
                        content = post.get('content', '').strip() if post.get('content') else 'No content available'
                        st.write(f"**Content:**\n{content}")
