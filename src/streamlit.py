...
class StockSentimentDashboard:
    ...
    def show_prediction_tab(self, df: pd.DataFrame, ticker: str):
        """Show stock movement predictions"""
        ...
        if not hasattr(self, "stock_analyzer"):
            try:
        ...
        # Get historical price data separately
        historical_data = self.stock_analyzer._get_historical_data(ticker)
        ...
        timeframe = st.radio(
            ...
            ["short", "medium", "long"],
            horizontal=True,
            help="Short: 2-3 days, Medium: 1-2 weeks, Long: 1 month",
        )

        # Get prediction
        with st.spinner("Analyzing market data..."):
            prediction = self.stock_analyzer.predict_stock_movement(ticker, timeframe)

        if not prediction.get("success"):
            st.error(f"Unable to generate prediction: {prediction.get('error')}")
            return
        ...
        with col1:
            # Get signal direction based on total score
            score = (
                prediction.get("signals", {}).get("scores", {}).get("total_score", 0)
            )
            direction = "bullish" if score > 0 else "bearish"
            direction_color = "green" if direction == "bullish" else "red"
            st.markdown(
                f"### Direction: <span style='color:{direction_color}'>{direction.upper()}</span>",
                unsafe_allow_html=True,
            )
        ...
        with col2:
            ...
            confidence = (
                prediction.get("signals", {}).get("scores", {}).get("confidence", 0)
            )
            st.metric("Confidence Score", f"{confidence:.1%}")

        with col3:
            # Get magnitude (absolute value of score)
            magnitude = abs(score)
            st.metric("Signal Strength", f"{magnitude:.2f}")

        # Show price chart with prediction
        if not historical_data.empty:
            ...
            future_dates = pd.date_range(
                start=historical_data.index[-1], periods=30, freq="D"
            )

            # Create prediction range
            predicted_change = score * 0.1  # Scale factor for visualization
            upper_bound = last_price * (1 + abs(predicted_change))
            lower_bound = last_price * (1 - abs(predicted_change))

            fig.add_trace(
                go.Scatter(
                    x=future_dates,
                    y=[
                        last_price * (1 + predicted_change * i / len(future_dates))
                        for i in range(len(future_dates))
                    ],
                    name="Predicted Trend",
                    line=dict(
                        color="green" if direction == "bullish" else "red", dash="dash"
                    ),
                )
            )

            fig.update_layout(
                title=f"{ticker} Price Trend with Prediction",
                xaxis_title="Date",
                yaxis_title="Price ($)",
                showlegend=True,
            )
        ...
        with st.expander("View Detailed Analysis"):
            ...
            tech_tab, sent_tab, mom_tab = st.tabs(
            ...
            with tech_tab:
                ...
                for category, values in tech_signals.items():
                    ...
                    if isinstance(values, dict):
                        ...
                        st.write(f"- Value: {values}")

            with sent_tab:
                st.write("### Sentiment Signals")
                sent_signals = signals.get("sentiment", {})
                for category, values in sent_signals.items():
                    st.write(f"**{category.title()}:**")
                    if isinstance(values, dict):
                        for name, value in values.items():
                            if isinstance(value, (int, float)):
            ...
            with mom_tab:
                ...
                mom_signals = signals.get("momentum", {})
                for category, values in mom_signals.items():
                    st.write(f"**{category.title()}:**")
                    if isinstance(values, dict):
                        for name, value in values.items():
                            if isinstance(value, (int, float)):
        ...
        st.write("### Summary")
        timeframe_map = {"short": "2-3 days", "medium": "1-2 weeks", "long": "1 month"}
        ...
        st.markdown(
            f"""
        Based on the analysis of technical indicators, sentiment signals, and momentum factors,
        {ticker} shows a **{direction}** trend over the next {timeframe_map[timeframe]} with 
        a confidence level of **{confidence:.1%}**. The signal strength of **{magnitude:.2f}** 
        indicates a {strength_desc} movement.
        """
        )
    ...
    def show_stock_details(self, ticker: str):
        ...
        with tab6:
            self.show_prediction_tab(stock_data, ticker)
