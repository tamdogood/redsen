...
@task(retries=3, retry_delay_seconds=60, cache_key_fn=task_input_hash, cache_expiration=timedelta(hours=6))
...
@task
def calculate_market_correlation(df: pd.DataFrame, market_data, start_date: dt.datetime, end_date: dt.datetime) -> pd.DataFrame:
...
def reddit_sentiment_flow():
    ...
    for subreddit in subreddits_to_analyze:
        ...
        if not result.empty:
            result = calculate_market_correlation(result, market_data, data_start_date, data_end_date)
